...
# Extended set for Crypto Portfolio Ecosystem (FastAPI + React + Python)
ALLOWED_COMMANDS = {
    # File inspection
    "ls",
    "cat",
    ...
    "deploy-fly.sh",  # Fly.io deployment script; validated separately
    "bash",
    "sh",
}

# Commands that need additional validation even when in the allowlist
COMMANDS_NEEDING_EXTRA_VALIDATION = {"pkill", "chmod", "init.sh", "deploy-fly.sh"}
...
def validate_deploy_fly_script(command_string: str) -> tuple[bool, str]:
    """
    Validate deploy-fly.sh script execution.

    Only allows:
    - ./scripts/deploy-fly.sh
    - paths ending in /scripts/deploy-fly.sh
    ...
    try:
        ...
        return False, "Could not parse deploy script command"
    ...
    # Allow ./scripts/deploy-fly.sh or paths ending in /scripts/deploy-fly.sh
    if script == "./scripts/deploy-fly.sh" or script.endswith("/scripts/deploy-fly.sh"):
        return True, ""

    # Also allow if called with bash explicitly
    if script == "bash" and len(tokens) > 1:
        script_arg = tokens[1]
        if script_arg == "./scripts/deploy-fly.sh" or script_arg.endswith("/scripts/deploy-fly.sh"):
            return True, ""

    return False, f"Only ./scripts/deploy-fly.sh is allowed, got: {script}"
    ...
    for cmd in commands:
        ...
        if cmd in COMMANDS_NEEDING_EXTRA_VALIDATION:
            ...
            if cmd == "pkill":
                ...
                if not allowed:
                    return {"decision": "block", "reason": reason}
            elif cmd == "deploy-fly.sh":
                allowed, reason = validate_deploy_fly_script(cmd_segment)
                if not allowed:
                    return {"decision": "block", "reason": reason}
